...
create_wrapper() {
    ...
    cat > "$install_path/$WRAPPER_NAME" << EOF
#!/bin/bash
...
EOF
    ...
    chmod +x "$install_path/$WRAPPER_NAME"
}
...
install_system_wide() {
    ...
    if [ -w "$INSTALL_DIR" ]; then
        ...
        sudo bash -c "$(declare -f create_wrapper); create_wrapper '$INSTALL_DIR' '$script_full_path'"
    fi
...
main() {
    echo "=================================="
    echo "  Architect CLI Installation"
    echo "=================================="
    echo
    ...
    echo "  2) Local installation (no sudo required)"
    echo "  3) Cancel"
    echo
